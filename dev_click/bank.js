...
function inBankInterest() {
  return (Number(player.inBank) * player.interestRate);
}
  ...
  if (depAmount == null) {
    ...
    if (investEntry > 0) {
      if (player.dollars >= investEntry) {
        ...
        $('#intPer10').text(comma(Math.floor(inBankInterest())));
      }
    }
  } else {
...
function investInterest() {
  ...
  if ((player.inBank + (inBankInterest())) < maxBalance) {

    player.totalInterest = Math.floor(player.totalInterest + (inBankInterest()));
    player.inBank = Math.floor(Number(player.inBank) + (inBankInterest()));

    $('#totalInterest').text(comma(player.totalInterest.toFixed(0)));
    $('#inBank').text(comma(player.inBank.toFixed(0)));
    $('#intPer10').text(comma(Math.floor(inBankInterest())));
  } else if (player.inBank < maxBalance && ((player.inBank + (inBankInterest())) >= maxBalance)) {
...
function increaseInterest() {
  player.interestRate = (player.interestRate + 0.001);
