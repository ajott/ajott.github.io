...
function moneyClick(numClicks) {
  getMoney(player.clickPower * player.karmaMult +
    Math.floor((player.clickUpgrades * 0.1 * MPS).toFixed(0)));
  player.clicks += 1;
  validateButtons();
}
...
function updateMPS() {
  ...
  for (i = 0; i < (player.workers.length); i += 1) {
    MPS = MPS +
      (player.workers[i] * player.workerProds[i] * player.workerMults[i]);
  }
  if (player.clickUpgrades > 0) {
    player.clickPowString = " (" +
      (player.clickPower * player.karmaMult).toString() + " + " +
      (player.clickUpgrades * 10).toString() + "% of MPS)";
  }
...
function comma(x) {
  if (x > 999) {
    return (Math.floor(parseFloat(x))).toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
...
function togglePanel(panel) {
  var panels = ['#staff', '#bankTable', '#charityTable',
    '#optionsPanel', '#statsPanel', '#upgradePanel'
  ];
  var tabs = ['#staffTab', '#bankTab', '#charityTab',
    '#optionsTab', '#statsTab', '#upgradeTab'
  ];
